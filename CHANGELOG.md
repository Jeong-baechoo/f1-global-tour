...
### Added
- 📅 **2026 시즌 캘린더 추가**
  - 2026 F1 시즌 레이스 일정 데이터 추가
  - 기존 코드 정리 및 최적화
- 🎬 **레이스 리플레이 시스템 대규모 개선**
  - 리플레이 전반적인 기능 확장 및 안정성 향상
  - 대규모 시스템 리팩터링으로 코드 구조 개선
- 🔗 **백엔드 API 연동**
  - 드라이버 텔레메트리 백엔드 연동 및 드라이버 선택 버그 수정
  - 플래그 인포 패널 백엔드 연동

### Fixed
- 🔒 **Next.js 15.3.4 → 15.5.14 보안 취약점 업데이트**
  - React Server Components 관련 CVE 취약점 대응
- 🔧 **eslint-config-next v16 마이그레이션 및 린트 에러 수정**
  - ESLint 9 flat config 체계로 전환
  - 마이그레이션으로 인한 린트 에러 일괄 수정
- 🐛 **빌드 오류 수정**
  - 미사용 변수 및 any 타입 제거

### Refactored
- 🏗️ **리플레이 모듈 전체 코드리뷰 및 리팩토링**
  - 리플레이 시스템 코드 품질 전면 개선
  - 리플레이 README 리팩토링 결과 반영

## [0.7.1] - 2025-09-07

### Fixed
- 🐛 **빌드 에러 수정 및 리플레이 패널 반응형 개선**
  - 리플레이 패널 모바일/데스크톱 반응형 레이아웃 개선

### Refactored
- 🏗️ **리플레이 트랙 정보 시스템 리팩토링 및 안정성 개선**
  - 트랙 정보 토글 시스템 안정화
  - 코드 구조 개선 및 에러 처리 강화

### Docs
- 📝 문서 업데이트 및 프로젝트 히스토리 정리
