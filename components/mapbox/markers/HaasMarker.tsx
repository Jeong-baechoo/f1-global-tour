...
interface HaasMarkerProps {
  map: mapboxgl.Map;
  team: Team;
  onMarkerClick?: (item: MarkerData) => void;
}

export const createHaasMarker = ({ map, team, onMarkerClick }: HaasMarkerProps): mapboxgl.Marker => {
  ...
  el.className = 'marker haas-marker';
  ...
  box.style.backgroundImage = 'url(/team-logos/MoneyGram_Haas_F1_Team_Logo.svg.png)';
  box.style.backgroundSize = 'contain';
  box.style.backgroundPosition = 'center';
  box.style.backgroundRepeat = 'no-repeat';
  box.style.backgroundColor = '#B6BABD'; // Haas silver
  box.style.borderRadius = MARKER_STYLES.redBullMarker.borderRadius;
  box.style.border = '2px solid #EC1B23'; // Haas red border
  box.style.boxShadow = '0 2px 10px rgba(182,186,189,0.4)'; // Haas silver shadow
  ...
  el.addEventListener('mouseenter', () => {
    ...
    box.style.boxShadow = '0 4px 20px rgba(236,27,35,0.6)'; // Enhanced Haas red shadow
  });

  el.addEventListener('mouseleave', () => {
    box.style.transform = 'scale(1) translateZ(0)';
    box.style.boxShadow = '0 2px 10px rgba(182,186,189,0.4)';
  });
  ...
  el.addEventListener('click', () => {
    ...
    map.flyTo({
      center: haasHQ.coordinates as [number, number],
      zoom: 15.68,
      pitch: 45,
      bearing: 0,
      speed: 0.4,
  ...
  return new mapboxgl.Marker(el, { 
    ...
    .setLngLat(haasHQ.coordinates as [number, number])
    .addTo(map);
};