...
interface RacingBullsMarkerProps {
  map: mapboxgl.Map;
  team: Team;
  onMarkerClick?: (item: MarkerData) => void;
}

export const createRacingBullsMarker = ({ map, team, onMarkerClick }: RacingBullsMarkerProps): mapboxgl.Marker => {
  ...
  el.className = 'marker racing-bulls-marker';
  ...
  box.style.backgroundImage = 'url(/team-logos/VCARB_F1_logo.svg)';
  box.style.backgroundSize = 'contain';
  box.style.backgroundPosition = 'center';
  box.style.backgroundRepeat = 'no-repeat';
  box.style.backgroundColor = '#2B4562'; // Racing Bulls navy blue
  box.style.borderRadius = MARKER_STYLES.redBullMarker.borderRadius;
  box.style.border = '2px solid #1A2B3D'; // Dark navy border
  box.style.boxShadow = '0 2px 10px rgba(43,69,98,0.4)'; // Racing Bulls navy shadow
  ...
  el.addEventListener('mouseenter', () => {
    ...
    box.style.boxShadow = '0 4px 20px rgba(43,69,98,0.6)'; // Enhanced Racing Bulls navy shadow
  });

  el.addEventListener('mouseleave', () => {
    box.style.transform = 'scale(1) translateZ(0)';
    box.style.boxShadow = '0 2px 10px rgba(43,69,98,0.4)';
  });
  ...
  el.addEventListener('click', () => {
    ...
    map.flyTo({
      center: racingBullsHQ.coordinates as [number, number],
      zoom: 15.68,
      pitch: 45,
      bearing: 0,
      speed: 0.4,
  ...
  return new mapboxgl.Marker(el, { 
    ...
    .setLngLat(racingBullsHQ.coordinates as [number, number])
    .addTo(map);
};