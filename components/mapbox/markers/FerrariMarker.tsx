...
interface FerrariMarkerProps {
  map: mapboxgl.Map;
  team: Team;
  onMarkerClick?: (item: MarkerData) => void;
}

export const createFerrariMarker = ({ map, team, onMarkerClick }: FerrariMarkerProps): mapboxgl.Marker => {
  ...
  el.className = 'marker ferrari-marker';
  ...
  box.style.backgroundImage = 'url(https://upload.wikimedia.org/wikipedia/de/thumb/c/c0/Scuderia_Ferrari_Logo.svg/200px-Scuderia_Ferrari_Logo.svg.png)';
  box.style.backgroundSize = 'contain';
  box.style.backgroundPosition = 'center';
  box.style.backgroundRepeat = 'no-repeat';
  box.style.backgroundColor = '#DC0000'; // Ferrari red
  box.style.borderRadius = MARKER_STYLES.redBullMarker.borderRadius;
  box.style.border = '2px solid #8B0000'; // Dark red border
  box.style.boxShadow = '0 2px 10px rgba(220,0,0,0.4)'; // Ferrari red shadow
  ...
  el.addEventListener('mouseenter', () => {
    ...
    box.style.boxShadow = '0 4px 20px rgba(220,0,0,0.6)'; // Enhanced Ferrari red shadow
  });

  el.addEventListener('mouseleave', () => {
    box.style.transform = 'scale(1) translateZ(0)';
    box.style.boxShadow = '0 2px 10px rgba(220,0,0,0.4)';
  });
  ...
  el.addEventListener('click', () => {
    ...
    map.flyTo({
      center: ferrariHQ.coordinates as [number, number],
      zoom: 15.68,
      pitch: 45,
      bearing: 0,
      speed: 0.4,
  ...
  return new mapboxgl.Marker(el, { 
    ...
    .setLngLat(ferrariHQ.coordinates as [number, number])
    .addTo(map);
};