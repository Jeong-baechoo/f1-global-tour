...
interface WilliamsMarkerProps {
  map: mapboxgl.Map;
  team: Team;
  onMarkerClick?: (item: MarkerData) => void;
}

export const createWilliamsMarker = ({ map, team, onMarkerClick }: WilliamsMarkerProps): mapboxgl.Marker => {
  ...
  el.className = 'marker williams-marker';
  ...
  box.style.backgroundImage = 'url(/team-logos/Williams_Racing_2022_logo.svg.png)';
  box.style.backgroundSize = 'contain';
  box.style.backgroundPosition = 'center';
  box.style.backgroundRepeat = 'no-repeat';
  box.style.backgroundColor = '#005AFF'; // Williams blue
  box.style.borderRadius = MARKER_STYLES.redBullMarker.borderRadius;
  box.style.border = '2px solid #004ACC'; // Dark blue border
  box.style.boxShadow = '0 2px 10px rgba(0,90,255,0.4)'; // Williams blue shadow
  ...
  el.addEventListener('mouseenter', () => {
    ...
    box.style.boxShadow = '0 4px 20px rgba(0,90,255,0.6)'; // Enhanced Williams blue shadow
  });

  el.addEventListener('mouseleave', () => {
    box.style.transform = 'scale(1) translateZ(0)';
    box.style.boxShadow = '0 2px 10px rgba(0,90,255,0.4)';
  });
  ...
  el.addEventListener('click', () => {
    ...
    map.flyTo({
      center: williamsHQ.coordinates as [number, number],
      zoom: 15.68,
      pitch: 45,
      bearing: 0,
      speed: 0.4,
  ...
  return new mapboxgl.Marker(el, { 
    ...
    .setLngLat(williamsHQ.coordinates as [number, number])
    .addTo(map);
};