...
interface MercedesMarkerProps {
  map: mapboxgl.Map;
  team: Team;
  onMarkerClick?: (item: MarkerData) => void;
}

export const createMercedesMarker = ({ map, team, onMarkerClick }: MercedesMarkerProps): mapboxgl.Marker => {
  ...
  el.className = 'marker mercedes-marker';
  ...
  box.style.backgroundImage = 'url(https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/Mercedes_AMG_Petronas_F1_Logo.svg/200px-Mercedes_AMG_Petronas_F1_Logo.svg.png)';
  box.style.backgroundSize = 'contain';
  box.style.backgroundPosition = 'center';
  box.style.backgroundRepeat = 'no-repeat';
  box.style.backgroundColor = '#00D2BE'; // Mercedes turquoise
  box.style.borderRadius = MARKER_STYLES.redBullMarker.borderRadius;
  box.style.border = '2px solid #008B8B'; // Dark turquoise border
  box.style.boxShadow = '0 2px 10px rgba(0,210,190,0.4)'; // Mercedes turquoise shadow
  ...
  el.addEventListener('mouseenter', () => {
    ...
    box.style.boxShadow = '0 4px 20px rgba(0,210,190,0.6)'; // Enhanced Mercedes turquoise shadow
  });

  el.addEventListener('mouseleave', () => {
    box.style.transform = 'scale(1) translateZ(0)';
    box.style.boxShadow = '0 2px 10px rgba(0,210,190,0.4)';
  });
  ...
  el.addEventListener('click', () => {
    ...
    map.flyTo({
      center: mercedesHQ.coordinates as [number, number],
      zoom: 15.68,
      pitch: 45,
      bearing: 0,
      speed: 0.4,
  ...
  return new mapboxgl.Marker(el, { 
    ...
    .setLngLat(mercedesHQ.coordinates as [number, number])
    .addTo(map);
};