...
### 🔍 적용했으나 특정 코너 검토 중 (3개) — 나중에 TPS로 일괄 처리

세 트랙 모두 **"특정 코너 한 곳만 전역 변환(어파인/2차)으로 안 잡히는"** 동일 패턴. GeoJSON은 정확하고(확인됨), 5명 드라이버가 같은 라인 → 원인은 **그 코너의 뱅킹/레이싱 라인** 또는 **OpenF1 좌표 국소 비선형**. 일단 적용해두고 나중에 손봄.

| 트랙 | 변환 | 중앙 | 문제 코너 | 비고 |
|---|---|---|---|---|
| italy | 2차 | 4.9m | 1번 시케인·Parabolica (90% 29m) | 2차→3차 미미, resample 균등은 더 나쁨 |
| monaco | 어파인 | ~4m | 일부 직선/코너 | 2차로도 개선 안 됨(어파인≈2차) |
| netherlands | 2차 | 5.7m | 1번 코너 Tarzan (뱅킹, 최대 44m) | 2차로 전반 개선했으나 그 코너는 잔존 |

**근본 해결책 = TPS (Thin-Plate Spline, 국소 비선형 변환)**:
- 장점: 모든 코너를 트랙에 정확히 붙임
- 트레이드오프: 마커를 실제 주행 위치에서 **트랙 중심선 쪽으로 약간 당김**(레이싱 라인 무시), 매 프레임 보간 → 약간의 런타임 비용
- **결정 대기**: 나머지 트랙 다 돌려본 뒤, 비슷한 코너 문제 트랙을 모아 TPS 일괄 도입 검토.
...
- **확정 21개**(어파인) + **검토 중 3개**(monaco 어파인, italy·netherlands 2차) = 24개 전부 PoC 육안 확인.
- 검토 중 3개는 특정 코너만 다항식 한계(뱅킹/레이싱 라인) → 나중에 TPS 일괄 도입 검토.
